{
  ...
  gpgme_1_24_0 = gpgme.overrideAttrs (
    finalAttrs: previousAttrs: {
      ...
      patches =
        if lib.versionAtLeast previousAttrs.version finalAttrs.version
        then previousAttrs.patches
        else lib.lists.drop 1 previousAttrs.patches;
      postPatch =
        if lib.versionAtLeast previousAttrs.version finalAttrs.version
        then previousAttrs.postPatch
        else null;
    }
  );
in
