...
stdenv.mkDerivation (finalAttrs: {
  ...
  cmakeFlags = [
    ...
    # map Nix store sources to the FetchContent names
    (lib.cmakeFeature "FETCHCONTENT_SOURCE_DIR_FSST" "${finalAttrs.passthru.deps.fsst}")
    (lib.cmakeFeature "FETCHCONTENT_SOURCE_DIR_RE2" "${finalAttrs.passthru.deps.re2}")
    (lib.cmakeFeature "FETCHCONTENT_SOURCE_DIR_GOOGLETEST" "${finalAttrs.passthru.deps.googletest}")
    (lib.cmakeFeature "FETCHCONTENT_SOURCE_DIR_NLOHMANN-JSON" "${finalAttrs.passthru.deps.nlohmann-json
    }")
    (lib.cmakeFeature "FETCHCONTENT_SOURCE_DIR_ANTLR" "${finalAttrs.passthru.deps.antlr}")
    (lib.cmakeFeature "FETCHCONTENT_SOURCE_DIR_RANGE-V3" "${finalAttrs.passthru.deps.range-v3}")
    (lib.cmakeFeature "FETCHCONTENT_SOURCE_DIR_SPATIALJOIN" "${finalAttrs.passthru.deps.spatialjoin}")
    (lib.cmakeFeature "FETCHCONTENT_SOURCE_DIR_CTRE" "${finalAttrs.passthru.deps.ctre}")
    (lib.cmakeFeature "FETCHCONTENT_SOURCE_DIR_ABSEIL" "${finalAttrs.passthru.deps.abseil}")
    (lib.cmakeFeature "FETCHCONTENT_SOURCE_DIR_S2" "${finalAttrs.passthru.deps.s2}")
  ];
