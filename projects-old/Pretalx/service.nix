...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    environment.etc."pretalx/pretalx.cfg".text =
      let
        hiddenNames = [
          "enable"
          "passwordFile"
        ...
        ifEnable = s: if s.enable then s else { };
        pretalxCfg =
      ...
      in
      {
        ...
        services =
          let
            ...
            exportPasswordEnv = concatStringsSep "\n" (
              ...
              ++ optional (cfg.database.passwordFile != null) (
                catFile "PRETALX_DB_PASS" cfg.database.passwordFile
              )
          ...
          in
          {
            ...
            pretalx-worker = mkIf cfg.celery.enable (
              recursiveUpdate commonUnitConfig {
