...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.liberaforms = {
      ...
      path = with pkgs; [ postgresql_11 ngipkgs.liberaforms-env openssl ];
      ...
      serviceConfig = {
        ...
        ExecStartPre = pkgs.writeScript "liberaforms-init" ''
          ...
          command = '${ngipkgs.liberaforms-env}/bin/gunicorn'
          bind = '${cfg.bind}'
          ...
          flask db upgrade
        '';
        ExecStart = "${ngipkgs.liberaforms-env}/bin/gunicorn -c ${cfg.workDir}/gunicorn.py 'wsgi:create_app()'";
