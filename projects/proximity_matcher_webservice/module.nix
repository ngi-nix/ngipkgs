...
let
  cfg = config.services.proximity_matcher_webservice;
in
{
  options.services.proximity_matcher_webservice = {
    enable = lib.mkEnableOption "proximity_matcher_webservice";
  ...
  config = {
    systemd.services.proximity_matcher_webservice = lib.mkIf cfg.enable {
      ...
      serviceConfig = {
        ...
        # from systemd-analyze --no-pager security proximity_matcher_webservice.service
