...
rec {
  ...
  examples = lib.mapAttrs (name: project: {
    services = lib.pipe project.nixos.modules.services [ (lib.mapAttrs (name: value: value.examples)) ];
    programs = lib.pipe project.nixos.modules.programs [ (lib.mapAttrs (name: value: value.examples)) ];
  }) projects;
