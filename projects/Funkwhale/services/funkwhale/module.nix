...
in
{
  options.services.funkwhale = {
    ...
    user = lib.mkOption {
      type = lib.types.str;
      default = "funkwhale";
      example = "funkkierwhale";
      description = "User account under which Funkwhale runs.";
    };
    group = lib.mkOption {
      type = lib.types.str;
      default = "funkwhale";
      example = "funkkierwhale";
      description = "Group under which Funkwhale runs.";
    };
  ...
  config = lib.mkIf cfg.enable {
    ...
    users.users = lib.mkIf (cfg.user == "funkwhale") {
      ${cfg.user} = {
        isSystemUser = true;
        group = cfg.group;
      };
    };
    users.groups = lib.mkIf (cfg.group == "funkwhale") {
      ${cfg.group} = { };
    };
    ...
    environment.systemPackages = [
      (pkgs.writeShellScriptBin "funkwhale-manage" ''
        if [[ ! "$USER" == "${cfg.user}" ]]; then
          echo "This has to be run with the \`${cfg.user}\` user. Ex: \`sudo -u ${cfg.user} funkwhale-manage\`"
          exit 1
        fi
        ...
        stat . &> /dev/null || echo -e \
          "WARNING: funkwhale-manage should be run from a directory the ${cfg.user} user has read access to.\n"
    ...
    systemd.tmpfiles.rules = [
      # Where generated secrets will be.
      "d /var/lib/funkwhale/config 0700 ${cfg.user} ${cfg.group} - -"

      # Niscellaneous API state files.
      "d /var/lib/funkwhale/api 0700 ${cfg.user} ${cfg.group} - -"

      # Files served by the API.
      "d /var/lib/funkwhale/data 0755 ${cfg.user} ${cfg.group} - -"
      # Default `MEDIA_ROOT`.
      "d /var/lib/funkwhale/data/media 0755 ${cfg.user} ${cfg.group} - -"
      # Default `MUSIC_DIRECTORY_PATH`.
      "d /var/lib/funkwhale/data/music 0755 ${cfg.user} ${cfg.group} - -"
    ];
