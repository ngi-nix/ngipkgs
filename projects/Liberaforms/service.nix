...
let
  ...
  pythonEnv = cfg.package.pythonModule.withPackages (ps: [ cfg.package ]);
in
{
    ...
    systemd.services.liberaforms = {
      ...
      serviceConfig = {
        ...
        ExecStartPre = pkgs.writeScript "liberaforms-init" /* bash */ ''
          ...
          command = '${pythonEnv}/bin/gunicorn'
          bind = '${cfg.bind}'
          ...
          flask db upgrade
        '';
        ExecStart = "${pythonEnv}/bin/gunicorn --preload --log-level debug -c ${cfg.workDir}/gunicorn.py 'wsgi:create_app()'";
