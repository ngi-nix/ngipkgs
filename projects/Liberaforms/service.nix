...
in
{
  options.services.liberaforms = {
    ...
    pythonEnv = mkOption {
      type = types.package;
      description = ''
        Final pythonEnv which is created from the `package`
      '';
      defaultText = "cfg.package.pythonModule.withPackages (_: [ cfg.package ]);";
      default = cfg.package.pythonModule.withPackages (_: [ cfg.package ]);
      readOnly = true;
    };
    ...
    smtp = {
      enable = mkEnableOption "SMTP email configuration";

      host = mkOption {
        type = types.str;
        default = "127.0.0.1";
        description = "SMTP provider FQDN or IP address.";
      };
      ...
      passwordFile = mkOption {
        ...
        default = null;
        description = "Path to a file containing the SMTP password. Recommended over storing secrets in plaintext.";
      };
      ...
      noreply = mkOption {
        ...
        description = "Email address used as the sender for outgoing automated emails.";
      };
    };
  };
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.liberaforms = {
      ...
      serviceConfig = {
        ...
        ExecStartPre = pkgs.writeScript "liberaforms-init" /* bash */ ''
          #!/bin/sh
          #set -x
          ...
          command = '${cfg.pythonEnv}/bin/gunicorn'
          bind = '${cfg.bind}'
          ...
          # allow writing to liberaforms, flask-webassets tries to write to liberaforms/static/.webassets-cache
          if [[ -L "./liberaforms" ]]; then
            rm ./liberaforms
            cp -rL ${cfg.package}/liberaforms .
            chmod -R u+w ./liberaforms
          fi
          ...
          ${lib.optionalString cfg.smtp.enable ''
            SMTP_ARGS=(
              ...
              -noreply "${cfg.smtp.noreply}"
            )

            ${lib.optionalString (cfg.smtp.user != "") ''
              SMTP_ARGS+=(-user "${cfg.smtp.user}")
            ''}

            ${lib.optionalString (cfg.smtp.encryption != "") ''
              SMTP_ARGS+=(-encryption "${cfg.smtp.encryption}")
            ''}

            ${lib.optionalString (cfg.smtp.passwordFile != null) ''
              SMTP_ARGS+=(-password "$(cat ${cfg.smtp.passwordFile})")
            ''}

            flask smtp set "''${SMTP_ARGS[@]}"
          ''}
        '';
        ExecStart = "${cfg.pythonEnv}/bin/gunicorn --preload --log-level debug -c ${cfg.workDir}/gunicorn.py 'wsgi:create_app()'";
