{
  ...
  environment = {
    ...
    # but then uwsgi would be missing out on these variables.
    GI_TYPELIB_PATH = lib.makeSearchPathOutput "out" "lib/girepository-1.0" (with pkgs; [
      pango
      harfbuzz
      librsvg
      gdk-pixbuf
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.weblate = {
      ...
      serviceConfig = {
        ...
        in "${uwsgi}/bin/uwsgi --json ${jsonConfig}";
        Restart = "on-failure";
        KillSignal = "SIGTERM";
        WorkingDirectory = pkgs.weblate;
        StateDirectory = "weblate";
